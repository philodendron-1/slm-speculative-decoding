...
# r=8 temp=1.0 + DistillSpec 전체
DRAFT_CONFIGS = {
    "r=8 (Seq-KD)_temp1.0":  ("/data/sonnet18s/lora_aligned/final", [1.0], False),
    "DistillSpec":             ("/data/sonnet18s/lora_distillspec/final", [0.0, 0.5, 1.0], True),
}
# True = expand_vocab 먼저 (DistillSpec)
# False = LoRA 먼저, expand_vocab 나중 (r=8)
...
def load_draft(lora_path, expand_first=False):
...
results = {}
for name, (lora_path, temps, expand_first) in DRAFT_CONFIGS.items():
    print(f"\n Draft: {name} 로드 중...", flush=True)
    ...
    for temp in temps:
        ...
        results[name][f"temp={temp}"] = round(ar * 100, 2)
...
json.dump(results, open("/data/sonnet18s/ar_remaining_result.json", "w"), indent=2)
print("저장 완료: ar_remaining_result.json", flush=True)
